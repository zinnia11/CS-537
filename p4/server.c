...
int lookup(int pinum, char *name){
    ...
    int remain = inode->size / sizeof(dir_ent_t);
	int end = MFS_BLOCK_SIZE / sizeof(dir_ent_t);
	// go through all directory entries
    for(int i = 0; i < num_block; i++){
        if(remain < MFS_BLOCK_SIZE / sizeof(dir_ent_t)){
            end = remain;
        }
...
int read_file(inode_t *n, message_t *message) {
	...
	if (n->type == MFS_DIRECTORY) {
    	...
    	int remain = n->size / sizeof(dir_ent_t);
		int end = MFS_BLOCK_SIZE / sizeof(dir_ent_t);
		// go through all directory entries
