...
static void process_marithmetic_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    case DELTA_ITEM_CAS_MISMATCH:
        ...
        p += 3;
        break;
    case DIV_BY_ZERO: // will not happen
        break;
    }
...
static void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const bool incr) {
    ...
    case DELTA_ITEM_NOT_FOUND:
        ...
        out_string(c, "NOT_FOUND");
        break;
    case DIV_BY_ZERO: // will never happen
        break;
    case DELTA_ITEM_CAS_MISMATCH:
...
static void process_new_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const bool mult) {
    char temp[INCR_MAX_STORAGE_LEN];
    uint64_t delta;
    char *key;
    size_t nkey;
...
void process_command_ascii(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        if (strcmp(tokens[COMMAND_TOKEN].value, "mult") == 0) {
            ...
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_new_arithmetic_command(c, tokens, ntokens, 1);
        } else {
            out_string(c, "ERROR");
        }
    } else if (first == 'd') {
        if (strcmp(tokens[COMMAND_TOKEN].value, "delete") == 0) {
            ...
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_new_arithmetic_command(c, tokens, ntokens, 0);
#ifdef MEMCACHED_DEBUG
